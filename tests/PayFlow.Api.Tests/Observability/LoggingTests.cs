...
public sealed class LoggingTests
{
    ...
    public async Task CreatePaymentHandler_LogsPaymentSuccessWithStructuredPaymentAndTenantProperties()
    {
        ...
        tenantContext
            .SetupGet(context => context.CurrentTenant)
            .Returns(tenant);
        idempotencyService
            .Setup(service => service.GetCachedResponseAsync<CreatePaymentResult>(
                It.IsAny<string>(),
                It.IsAny<string>(),
        ...
        idempotencyService
            ...
            .ReturnsAsync(true);
        idempotencyService
            .Setup(service => service.CacheResponseAsync(
                It.IsAny<string>(),
                It.IsAny<string>(),
                It.IsAny<CreatePaymentResult>(),
                It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);
        idempotencyService
            .Setup(service => service.ReleaseLockAsync(
                It.IsAny<string>(),
                It.IsAny<string>(),
                It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);
        ...
        eventPublisher
            ...
            .Returns(Task.CompletedTask);
        transaction
            .Setup(dbTransaction => dbTransaction.CommitAsync(It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);
        transaction
            .Setup(dbTransaction => dbTransaction.RollbackAsync(It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);
        unitOfWork
        ...
        logger.Verify(
            log => log.Log(
                ...
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((state, _) => HasPaymentSucceededProperties(state, result.PaymentId, tenantId)),
