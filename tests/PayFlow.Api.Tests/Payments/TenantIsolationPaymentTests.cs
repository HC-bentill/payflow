...
public sealed class TenantIsolationPaymentTests(PayFlowApiFactory factory) : IClassFixture<PayFlowApiFactory>
{
    ...
    public async Task TenantB_CannotReadTenantAPayment()
    {
        ...
        var tenantA = await PaymentTestClient.CreateAuthenticatedClientAsync(
            factory,
            $"tenant-a-{Guid.NewGuid():N}");
        var tenantB = await PaymentTestClient.CreateAuthenticatedClientAsync(
            factory,
            $"tenant-b-{Guid.NewGuid():N}");
        var createResponse = await tenantA.Client.CreatePaymentAsync($"idem-{Guid.NewGuid():N}");
        var created = await createResponse.Content.ReadFromJsonAsync<PaymentResponse>(
            AuthTestClient.JsonOptions,
            CancellationToken.None);

        var getResponse = await tenantB.Client.GetAsync(
        ...
        var list = await listResponse.Content.ReadFromJsonAsync<ListPaymentsResponse>(
            AuthTestClient.JsonOptions,
            CancellationToken.None);
